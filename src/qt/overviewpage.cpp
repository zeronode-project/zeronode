...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString stranonymizeNODEAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, anonymizeNODEamount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        stranonymizeNODEAmount = stranonymizeNODEAmount.remove(stranonymizeNODEAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = stranonymizeNODEAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > anonymizeNODEamount * COIN) nMaxToAnonymize = anonymizeNODEamount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= anonymizeNODEamount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(stranonymizeNODEAmount));
        stranonymizeNODEAmount = stranonymizeNODEAmount.remove(stranonymizeNODEAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = stranonymizeNODEAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
