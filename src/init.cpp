...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "zeronode.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "zeronoded.pid"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, zeronode, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-anonymizeNODEamount=<n>", strprintf(_("Keep N NODE anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 33673, 51475));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The ColossusCoinXT Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The ZeroNode Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that ZeroNode is running in a usable environment with all
...
// ********************************************************* Step 8: load wallet
    ...
    anonymizeNODEamount = GetArg("-anonymizeNODEamount", 0);
    if (anonymizeNODEamount > 999999) anonymizeNODEamount = 999999;
    if (anonymizeNODEamount < 2) anonymizeNODEamount = 2;
    ...
    LogPrintf("Anonymize ZeroNode Amount %d\n", anonymizeNODEamount);
    ...
    /* Denominations
       ...
       For example:
       1NODE+1000 == (.1NODE+100)*10
       10NODE+10000 == (1NODE+1000)*10
    */
